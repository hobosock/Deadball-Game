    ...
    if *hit_result <= 2 {
        if batter.speedy() {
            ...
            if batter.contact_hit() {
                ...
                match state.inning_half {
                    InningTB::Top => {
                        state.away_state.hits += 1;
                    }
                    InningTB::Bottom => {
                        state.home_state.hits += 1;
                    }
                }
            }
        }
        ...
        match state.inning_half {
            InningTB::Top => {
                state.away_state.runs += runs;
            }
            InningTB::Bottom => {
                state.home_state.runs += runs;
            }
        }
        match state.inning_half {
            InningTB::Top => {
...
pub fn defense(
    ...
    if *def_result <= 2 {
        ...
        match state.inning_half {
            InningTB::Top => {
                state.away_state.hits -= 1;
                state.home_state.errors += 1;
            }
            InningTB::Bottom => {
                state.home_state.hits -= 1;
                state.away_state.errors += 1;
            }
        }
...
pub fn runners_advance(mut state: GameState, advance_num: &u32) -> GameState {
    if *advance_num == 1 {
        ...
        match state.runners {
            ...
            RunnersOn::Runner011 => {
                ...
                match state.inning_half {
                    InningTB::Top => {
                        // away team at bat, update team 2 score
                        state.away_state.runs += 2;
                    }
                    InningTB::Bottom => {
                        state.home_state.runs += 2;
                    }
                }
            }
            RunnersOn::Runner101 => {
        ...
        match state.inning_half {
            InningTB::Top => {
                // away team at bat, update team 2 score
                state.away_state.runs += num_runners;
            }
            InningTB::Bottom => {
                state.home_state.runs += num_runners;
            }
        }
    }
